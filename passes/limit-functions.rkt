...
(define ((limit-subfuncs var-ind-map vector-var-name) exp)
  (match exp
    ...
    [(SetBang x rhs)
     (if (hash-has-key? var-ind-map x)
         (Prim 'vector-set! (list (Var vector-var-name) (Int (hash-ref var-ind-map x)) ((limit-subfuncs var-ind-map vector-var-name) rhs)))
    ...
    [(Let x e body) (Let x ((limit-subfuncs var-ind-map vector-var-name) e) ((limit-subfuncs var-ind-map vector-var-name) body))]
    [(If e1 e2 e3) (If ((limit-subfuncs var-ind-map vector-var-name) e1) ((limit-subfuncs var-ind-map vector-var-name) e2) ((limit-subfuncs var-ind-map vector-var-name) e3))]
    [(Prim op es) (Prim op (map (limit-subfuncs var-ind-map vector-var-name) es))]
    [(WhileLoop cnd body) (WhileLoop ((limit-subfuncs var-ind-map vector-var-name) cnd) ((limit-subfuncs var-ind-map vector-var-name) body))]
    [(Begin es last-exp)
     (Begin (map (limit-subfuncs var-ind-map vector-var-name) es) ((limit-subfuncs var-ind-map vector-var-name) last-exp))]
    [(Apply func args)
     (if (> (length args) CNT)
         ...
         (Apply ((limit-subfuncs var-ind-map vector-var-name) func) (map (limit-subfuncs var-ind-map vector-var-name) args)))
         ]
    ))
  ...
  (match def
    [(Def name param rty info body)
     ...
     (define new-param
       (if (> (length param) CNT)
           (match param
             [`([,xs : ,ts] ...) (append (take param (- CNT 1)) `([,vector-var-name : (Vector ,@(drop ts (- CNT 1)))]))])
           param))
